...
function isHoliday(date: Date) {
  const iso = format(date, "yyyy-MM-dd"); // ✅ no timezone shift
...
export function getMonthCellEvents(
  ...
  // ✅ Holiday → return holiday marker
  const holidayName = isHoliday(date);
  if (holidayName) {
    return [
      {
        id: `holiday-${date.toISOString()}`,
        title: holidayName,
        isHoliday: true,
      },
    ];
  ...
  // ✅ Only match by day (ignores time zone issues)
  ...
  return eventsForDate.map((event) => ({
    ...
    isMultiDay: false, // since we already expand per-day in expandPlanToEvents
    isHoliday: false,
    dayOfEvent: date.toISOString(),
  }));
}
